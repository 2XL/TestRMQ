    ...
    def runtest(self):
        ...
        if self.is_warmup:
            ...
            while self.is_running:
                operations = 10000000 # executant de forma indefinida...
                for i in range(operations):
                    # self.stereotype_executor.execute()
                    time.sleep(3)
                    print colored("doOps {}/{}".format(i, operations), 'red')
        else:
            return 'WARNING: need warmup 1st!'
        return 'SUCCESS: run test response'

    def stoptest(self):
        if self.is_running:
            print 'stop test'
            self.is_running = False
            return 'SUCCESS: stop test'
        else:
            return 'WARNING: no test is running'
